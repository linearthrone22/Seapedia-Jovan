...
import { ArrowLeft, Clock, Store, MapPin, Truck, RefreshCw } from "lucide-react";
import { STATUS_BADGES } from "../page";
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-6">
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-2xl font-extrabold text-white">Order Tracking</h1>
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              <Card className="bg-slate-900/40 border-white/5 p-6">
                <h3 className="font-bold text-white text-md flex items-center gap-2 border-b border-white/5 pb-3">
                  <Clock className="h-4.5 w-4.5 text-teal-400" /> Shipment Timeline
                </h3>
              ...
              {/* 2. Items Purchased */}
              <Card className="bg-slate-900/40 border-white/5 p-6">
                <h3 className="font-bold text-white text-md flex items-center gap-2 border-b border-white/5 pb-3 mb-4">
                  <Store className="h-4.5 w-4.5 text-teal-400" /> Items List (Merchant: {order.store.name})
                </h3>
            ...
            <div className="md:col-span-1 space-y-6">
              ...
              <Card className="bg-slate-900/40 border-white/5 p-6 space-y-4">
                <h3 className="font-bold text-white text-sm flex items-center gap-1.5 border-b border-white/5 pb-2">
                  <MapPin className="h-4 w-4 text-teal-400" /> Shipping Destination
                </h3>
              ...
              <Card className="bg-slate-900/40 border-white/5 p-6 space-y-3">
                <h3 className="font-bold text-white text-sm flex items-center gap-1.5 border-b border-white/5 pb-2">
                  <Truck className="h-4 w-4 text-teal-400" /> Delivery Method
                </h3>
                <div className="text-xs">
                  ...
                  <p className="text-slate-500 text-[10px] mt-2 leading-relaxed">
                    SLA: {order.deliveryMethod === "INSTANT" ? "1 Day" : order.deliveryMethod === "NEXT_DAY" ? "2 Days" : "5 Days"}
                  </p>
                </div>
              </Card>
              ...
              <Card className="bg-slate-900/40 border-white/5 p-6 space-y-4">
                ...
                <div className="space-y-2.5 text-xs text-slate-400">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-slate-200">{formatPrice(order.deliveryFee)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>PPN Tax (12%)</span>
                    <span className="font-semibold text-slate-200">{formatPrice(order.taxAmount)}</span>
                  </div>
